...
public class CrateFactory {
    ...
    private static void registerItemCrate(ItemStack itemStack, String crateName) {
        ...
        ITEM_CRATE_MAP.put(itemWrapper, Pair.of(crateName, itemStack.getCount()));
    ...
    public static int getItemCrateCountRequired(ItemStack itemStack) {
        return Optional.ofNullable(ITEM_CRATE_MAP.get(new KujiCrateType.ItemWrapper(itemStack))).map(Pair::getY).orElse(1);
    }

    public static KujiObj.Crate tryGetItemCrate(ItemStack itemStack) {
        return LOADED_CRATES.getOrDefault(Optional.ofNullable(ITEM_CRATE_MAP.get(new KujiCrateType.ItemWrapper(itemStack))).map(Pair::getX).orElse(null), null);
    }
