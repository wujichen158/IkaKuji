...
public class GiveCmd {
    ...
    public void run(@Sender CommandSource sender,
                    ...
                    @Completable(PlayerTabCompleter.class) @Argument ServerPlayer targetPlayer,
        ...
        Optional.ofNullable(CrateFactory.get(crateName)).ifPresent(crate -> {
            ...
            if (CrateDeliverCompleter.KEY.equalsIgnoreCase(type)) {
                ...
                if (Optional.ofNullable(key).isPresent()) {
                    ...
                    sender.sendSystemMessage(MsgUtil.prefixedColorMsg(commands.getGiveKey(), crateName, targetPlayer.getName().getString()));
                } else {
                    sender.sendSystemMessage(MsgUtil.prefixedColorMsg(commands.getNoKey(), crateName));
                }
                ...
                if (crate.getCrateType() == EnumCrateType.item && CrateFactory.getAll().containsKey(crateName)) {
                    ...
                    if (Optional.ofNullable(itemStacks1).isPresent()) {
                        ...
                        sender.sendSystemMessage(MsgUtil.prefixedColorMsg(commands.getGiveCrate(), crateName, targetPlayer.getName().getString()));
                    } else {
                        sender.sendSystemMessage(MsgUtil.prefixedColorMsg(commands.getInvalidItemCrate(), crateName));
                    }
                } else {
                    sender.sendSystemMessage(MsgUtil.prefixedColorMsg(commands.getNotItemCrate(), crateName));
                }
            }
