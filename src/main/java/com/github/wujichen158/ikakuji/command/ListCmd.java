...
public class ListCmd {
    ...
    public void run(@Sender EntityPlayerMP player, String[] args) {
        Optional.ofNullable(PlayerKujiFactory.get(player.getUniqueID())).ifPresent(playerData -> {
            int onePageSize = 8;
            int page = 0;
            if (args.length > 0) {
                try {
            ...
            // in style
            player.sendMessage(MsgUtil.colorMsg(commands.getListTitle()));
            pagedCrateList.forEach(crateName ->
                    player.sendMessage(MsgUtil.colorMsg(commands.getListElemPrefix(), crateName)));
            for (int i = 0; i < onePageSize - pagedCrateList.size(); i++) {
                player.sendMessage(new TextComponentString(""));
            }
            player.sendMessage(MsgUtil.colorMsg(commands.getListFooter()));
        });
    }
}